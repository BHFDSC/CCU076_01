...
# Purpose:  2nd stage modelling and forest plot for venous thrombotic event.
#*******************************************************************************
...
# Load data (use the interaction export if that's where your p-values live)
# df <- read_excel("export/Oct_2025/first_stage_models.xlsx") |> filter(outcome=="ve_fatal")
df <- read_excel("export/Oct_2025/first_stage_models_with_interaction.xlsx") |> filter(outcome == "ve_fatal")
...
df$subgroup <- recode(
  df$subgroup,
  "COVID19 diagnosis within 6 mo before first event" =
    "COVID19 in last 6mo",
  "No COVID19 diagnosis within 6 mo before first event" =
    "No COVID19 in last 6mo"
)
...
subgroup_info <- list(
  "main_analysis" = NA,
  "COVID19_6mo" = c("COVID19 in last 6mo", "No COVID19 in last 6mo"),
  "sex" = c("Female", "Male"),
...
# ------------------------------
# Interaction p-value helpers
# ------------------------------
# Change this if your p-value column is named differently
pval_col <- "interaction_chisq"
...
# Get one p-value per analysis/grouping variable (NOT per subgroup)
...
# Function to build a p-value annotate layer (bold if significant)
p_annot <- function(x, y, analysis_name, alpha_sig = 0.05) {
  p <- get_p_for(analysis_name)
  if (is.na(p)) return(NULL)
  p <- as.numeric(p)
  
  lab <- paste0(fmt_pval(p))
...
# ------------------------------
# Function to process subgroup analysis (non-region)
# ------------------------------
...
vlines <- c(
  1.45, 3.45, 5.45, 7.45, 10.55, 12.55, 14.55, 16.55, 18.55, 21.45
)
...
# ---- PLOT ----
# Header y positions (log scale; must be within limits)
y_header <- 3.8
y_pval   <- 3.35  # underneath the header line
...
p <- ggplot(plot_df, aes(
  ...
  labs(
    ...
    title = "Incidence rate ratios for venous events across temperature extremes"
  ) +
...
# ---- SAVE ----
ggsave(
  filename = here::here("figures", "panel_forest_plot_IRR_venous_events_with_interaction.png"),
  plot = p,
